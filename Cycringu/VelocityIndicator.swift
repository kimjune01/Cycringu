...
class VelocityIndicator: UIView {
  ...
  var velocityIncrement:CGFloat = 0
  var velocityMultiplier:CGFloat = 20
  ...
  func displayLinkFired(link:CADisplayLink) {
    if didTick {
      let increasingAmount = velocityMultiplier*velocityIncrement
      velocity += Int(increasingAmount)
      println("increasingAmount: \(increasingAmount)")
    }
    ...
    if CGFloat(velocity) > screenHeight {
      println((velocity, screenHeight))
      velocity = Int(screenHeight)
      velocityIncrement = 0
  ...
  func tick(increment:CGFloat){
    didTick = true
    velocityIncrement = increment
//    if velocityIncrement > 0 {
//      velocityIncrement *= 0.9
//    }
